...
`timer/checkpoint_logger.py` is a small utility that tails the tracer data and writes one CSV row per checkpoint segment using room anchors. Run it with `python3 -m timer.checkpoint_logger` (or `python3 timer/checkpoint_logger.py`) while Celeste and the tracer are active. Output is written to `timer_data/checkpoint_logs/<timestamp>.csv`. Set `CHECKPOINT_LOGGER_DEBUG=1` to print state changes and segment starts/ends for troubleshooting.

Supported content currently includes the Prologue plus Forsaken City–Core A/B (standard vanilla rooms). Epilogue, Farewell, all C-sides, and any modded content are ignored. The logger relies on room names pulled straight from the game, so anchor mismatches, save-and-quit routing, and spreadsheet-specific semantics (e.g., 2A/5A Any% routing tricks) may need manual adjustments in future revisions.
